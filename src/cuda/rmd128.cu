...
__host__ void HC_GPU_FN(rmd128_on_gpu_prepare)(int device_ix, const unsigned char* dict, size_t dict_len,
...
__host__ static void prrmd128_run_kernel(gpu_tread_ctx_t* ctx, const size_t dict_len) {
    GPU_LAUNCH_INDEX_KERNEL(prrmd128_kernel, ctx, dict_len);
}

__host__ void HC_GPU_FN(rmd128_run_on_gpu)(gpu_tread_ctx_t* ctx, const size_t dict_len) {
    gpu_run(ctx, dict_len, &prrmd128_run_kernel);
}

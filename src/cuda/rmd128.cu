/*!
 * \brief   RIPEMD-128 CUDA brute-force (short passwords, one block).
 * \author  \verbatim
...
__global__ static void prrmd128_kernel(unsigned char* result, const uint64_t start, const uint32_t count,
                                       const uint32_t pass_len, const uint32_t dict_length, const uint32_t min_len);
__device__ static BOOL prrmd128_compare(unsigned char* password, const int length, uint8_t* hash);
__device__ static void prrmd128_hash(const uint8_t* message, size_t len, uint8_t* hash);
__device__ static void prrmd128_compress(uint32_t* state, const uint8_t* block);

__constant__ static unsigned char k_dict[CHAR_MAX];
__constant__ static unsigned char k_hash[HASH_LEN];

__host__ void rmd128_on_gpu_prepare(int device_ix, const unsigned char* dict, size_t dict_len,
...
__host__ static void prrmd128_run_kernel(gpu_tread_ctx_t* ctx, unsigned char* dev_result, unsigned char* dev_variants,
                                         const size_t dict_len) {
    (void)dev_result;
    (void)dev_variants;
    GPU_LAUNCH_INDEX_KERNEL(prrmd128_kernel, ctx, dict_len);
}

__host__ void rmd128_run_on_gpu(gpu_tread_ctx_t* ctx, const size_t dict_len, unsigned char* variants,
                                const size_t variants_size) {
    gpu_run(ctx, dict_len, variants, variants_size, &prrmd128_run_kernel);
}

KERNEL_WITH_ALLOCATION(prrmd128_kernel, prrmd128_compare, uint8_t, HASH_LEN)

__device__ __forceinline__ BOOL prrmd128_compare(unsigned char* password, const int length, uint8_t* hash) {
    prrmd128_hash(password, length, hash);
    BOOL result = TRUE;
...
__device__ __forceinline__ void prrmd128_hash(const uint8_t* message, size_t len, uint8_t* hash) {
    uint32_t state[4] = {
...
__device__ __forceinline__ void prrmd128_compress(uint32_t* state, const uint8_t* block) {
    uint32_t X[16];
#pragma unroll
    for (int j = 0; j < 16; j++) {
        const int i = j * 4;

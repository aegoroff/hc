/*!
 * \brief   Tiger / Tiger2 CUDA brute-force (short passwords, one block).
 * \author  \verbatim
            Created by: Alexander Egorov
            \endverbatim
 * Copyright: (c) Alexander Egorov 2009-2026
 *
 * Tiger and Tiger2 share compression; only the padding byte differs
 * (0x01 vs 0x80), matching sph_tiger / md_helper.c.
 */
...
__device__ __align__(16) static const uint64_t T1[256] = {
...
__device__ __align__(16) static const uint64_t T2[256] = {
...
__device__ __align__(16) static const uint64_t T3[256] = {
...
__device__ __align__(16) static const uint64_t T4[256] = {
...
__global__ static void prtiger_kernel(unsigned char* result, const uint64_t start, const uint32_t count,
                                      const uint32_t pass_len, const uint32_t dict_length, const uint32_t min_len);
__global__ static void prtiger2_kernel(unsigned char* result, const uint64_t start, const uint32_t count,
                                       const uint32_t pass_len, const uint32_t dict_length, const uint32_t min_len);
__device__ static void prtiger_hash(const uint8_t* message, size_t len, uint8_t* hash, uint8_t pad_byte,
...
__global__ void prtiger2_kernel(unsigned char* result, const uint64_t start, const uint32_t count,
                                const uint32_t pass_len, const uint32_t dict_length, const uint32_t min_len) {
    __shared__ uint64_t sT1[256], sT2[256], sT3[256], sT4[256];
    prtiger_load_sboxes(sT1, sT2, sT3, sT4);
...
__device__ __forceinline__ void prtiger_hash(const uint8_t* message, size_t len, uint8_t* hash, uint8_t pad_byte,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        hash[i * 8 + 0] = static_cast<uint8_t>(v);
        hash[i * 8 + 1] = static_cast<uint8_t>(v >> 8);
        hash[i * 8 + 2] = static_cast<uint8_t>(v >> 16);
        hash[i * 8 + 3] = static_cast<uint8_t>(v >> 24);
        hash[i * 8 + 4] = static_cast<uint8_t>(v >> 32);
        hash[i * 8 + 5] = static_cast<uint8_t>(v >> 40);
        hash[i * 8 + 6] = static_cast<uint8_t>(v >> 48);
        hash[i * 8 + 7] = static_cast<uint8_t>(v >> 56);
    }
}

#define MUL5(x) ((x) * UINT64_C(5))
#define MUL7(x) ((x) * UINT64_C(7))
#define MUL9(x) ((x) * UINT64_C(9))
...
__device__ __forceinline__ void prtiger_compress(uint64_t* state, const uint8_t* block,
                                                   const uint64_t* T1s, const uint64_t* T2s,
                                                   const uint64_t* T3s, const uint64_t* T4s) {
    uint64_t X0, X1, X2, X3, X4, X5, X6, X7;
#define LOAD64LE(off) ( \
    static_cast<uint64_t>(block[(off) + 0]) | \
    (static_cast<uint64_t>(block[(off) + 1]) << 8) | \
    (static_cast<uint64_t>(block[(off) + 2]) << 16) | \
    (static_cast<uint64_t>(block[(off) + 3]) << 24) | \
    (static_cast<uint64_t>(block[(off) + 4]) << 32) | \
    (static_cast<uint64_t>(block[(off) + 5]) << 40) | \
    (static_cast<uint64_t>(block[(off) + 6]) << 48) | \
    (static_cast<uint64_t>(block[(off) + 7]) << 56))
    X0 = LOAD64LE(0);
    X1 = LOAD64LE(8);
    X2 = LOAD64LE(16);
    X3 = LOAD64LE(24);

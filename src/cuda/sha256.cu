...
__global__ static void prsha256_kernel(unsigned char* result, const uint64_t start, const uint32_t count, const uint32_t pass_len, const uint32_t dict_length, const uint32_t min_len);
...
__host__ static void prsha256_run_kernel(gpu_tread_ctx_t* ctx, unsigned char* dev_result, unsigned char* dev_variants, const size_t dict_len);

__constant__ static unsigned char k_dict[CHAR_MAX];
__constant__ static unsigned char k_hash[DIGESTSIZE];

__host__ void sha256_on_gpu_prepare(int device_ix, const unsigned char* dict, size_t dict_len, const unsigned char* hash, gpu_tread_ctx_t* ctx) {
...
__host__ void prsha256_run_kernel(gpu_tread_ctx_t* ctx, unsigned char* dev_result, unsigned char* dev_variants, const size_t dict_len) {
    (void)dev_result;
    (void)dev_variants;
    GPU_LAUNCH_INDEX_KERNEL(prsha256_kernel, ctx, dict_len);
}
...
__host__ void sha256_run_on_gpu(gpu_tread_ctx_t* ctx, const size_t dict_len, unsigned char* variants, const size_t variants_size) {
    gpu_run(ctx, dict_len, variants, variants_size, &prsha256_run_kernel);
}

KERNEL_WITHOUT_ALLOCATION(prsha256_kernel, prsha256_compare)

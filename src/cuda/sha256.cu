...
__host__ static void prsha256_run_kernel(gpu_tread_ctx_t* ctx, const size_t dict_len);

__constant__ static unsigned char k_dict[CHAR_MAX];
__constant__ static unsigned char k_hash[DIGESTSIZE];

__host__ void HC_GPU_FN(sha256_on_gpu_prepare)(int device_ix, const unsigned char* dict, size_t dict_len, const unsigned char* hash, gpu_tread_ctx_t* ctx) {
...
__host__ void prsha256_run_kernel(gpu_tread_ctx_t* ctx, const size_t dict_len) {
    GPU_LAUNCH_INDEX_KERNEL(prsha256_kernel, ctx, dict_len);
}

__host__ void HC_GPU_FN(sha256_run_on_gpu)(gpu_tread_ctx_t* ctx, const size_t dict_len) {
    gpu_run(ctx, dict_len, &prsha256_run_kernel);
}

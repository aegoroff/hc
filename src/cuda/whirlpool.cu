...
__global__ static void prwhirl_kernel(unsigned char* result, const uint64_t start, const uint32_t count, const uint32_t pass_len, const uint32_t dict_length, const uint32_t min_len);
...
__host__ void HC_GPU_FN(whirl_on_gpu_prepare)(int device_ix, const unsigned char* dict, size_t dict_len, const unsigned char* hash, gpu_tread_ctx_t* ctx) {
...
__host__ void HC_GPU_FN(whirl_run_on_gpu)(gpu_tread_ctx_t* ctx, const size_t dict_len) {

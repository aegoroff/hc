...
__global__ static void prsha512_kernel(unsigned char* result, const uint64_t start, const uint32_t count, const uint32_t pass_len, const uint32_t dict_length, const uint32_t min_len);
...
__host__ static void prsha512_run_kernel(gpu_tread_ctx_t* ctx, const size_t dict_len);
...
__host__ void prsha512_run_kernel(gpu_tread_ctx_t* ctx, const size_t dict_len) {

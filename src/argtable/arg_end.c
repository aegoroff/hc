...
static void errorfn(void *parent, FILE *fp, int error, const char *argval, const char *progname)
    ...
    switch(error)
        ...
        case ARG_EMALLOC:
            ...
            break;
        case ARG_ENOMATCH:
            lib_fprintf(fp,"unexpected argument \"%s\"",argval);
            break;
        case ARG_EMISSARG:
            lib_fprintf(fp,"option \"%s\" requires an argument",argval);
            break;
        case ARG_ELONGOPT:
            lib_fprintf(fp,"invalid option \"%s\"",argval);
            break;
        default:
            lib_fprintf(fp,"invalid option \"-%c\"",error);
            break;
        }
    fputc('\n',fp);
    }

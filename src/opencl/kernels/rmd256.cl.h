...
"\n"\
"\n"\
"static void prrmd256_compress(uint* state, const uchar* block) {\n"\
"    uint X[16];\n"\
"    for (int j = 0; j < 16; j++) {\n"\
"        const int i = j * 4;\n"\
"        X[j] = (uint)(block[i + 0])\n"\
...
"    uint aaa = state[4], bbb = state[5], ccc = state[6], ddd = state[7];\n"\
"    uint tmp;\n"\
"\n"\
...
"}\n"\
"\n"\
"static void prrmd256_hash(const uchar* message, uint len, uchar* hash) {\n"\
"    uint state[8] = {\n"\
...
"}\n"\
"\n"\
"static int prrmd256_compare(__global const uchar* k_hash, uchar* password, const int length) {\n"\
"  uchar hash[32];\n"\
"  prrmd256_hash(password, (uint)length, hash);\n"\
"  for (int i = 0; i < 32; ++i) {\n"\
...
"\n"\
"\n"\
"__kernel void prrmd256_kernel(__global uchar* result,\n"\

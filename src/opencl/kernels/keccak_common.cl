/* Shared Keccak-f[1600] BF kernel body for SHA-3 / Keccak variants.
 * Callers must #define before this source (via a tiny head .cl #embed'd first):
 *   KECCAK_RATE, KECCAK_HASH_LEN, KECCAK_PAD, KECCAK_COMPARE, KECCAK_KERNEL
 *
...
static int KECCAK_COMPARE(__global const uchar* k_hash, uchar* password, const int length) {
  return prsha3_compare_reg(k_hash, password, length, KECCAK_RATE, KECCAK_HASH_LEN, KECCAK_PAD);
}

__kernel void KECCAK_KERNEL(__global uchar* result,
  ...
  for (uint i = 0; i < dict_length; ++i) {
    ...
    if (pass_len + 1u == 4u && pass_len + 1u >= min_len) {
      if (*g_found) return;
      if (KECCAK_COMPARE(k_hash, attempt, (int)(pass_len + 1u))) {
    ...
    for (uint j = 0; j < dict_length; ++j) {
      ...
      if (*g_found) return;
      if (KECCAK_COMPARE(k_hash, attempt, (int)(pass_len + 2u))) {

...
#define DIGESTSIZE 4
#define INITIALIZATION_VALUE 0xFFFFFFFFu

__constant uint crc_tab[] = {
    ...
    0x92d28e9bu, 0xe5d5be0du, 0x7cdcefb7u, 0x0bdbdf21u, 0x86d3d2d4u, 0xf1d4e246u,
...
static int prcrc32_compare(__global const uchar* k_hash, uchar* password, int length) {
  uint crc = INITIALIZATION_VALUE;
  const uchar* block = password;
  while (length--) {
    crc = ((crc >> 8) & 0x00FFFFFFu) ^ crc_tab[(crc ^ *block++) & 0xFFu];
  }
  crc = ~crc;
  return k_hash[0] == (uchar)(crc >> 24)
      && k_hash[1] == (uchar)(crc >> 16)
      && k_hash[2] == (uchar)(crc >> 8)
      && k_hash[3] == (uchar)crc;
}


__kernel void prcrc32_kernel(__global uchar* result,
  ...
  for (uint i = 0; i < dict_length; ++i) {
    ...
    if (pass_len + 1u == 4u && pass_len + 1u >= min_len) {
      if (*g_found) return;
      if (prcrc32_compare(k_hash, attempt, (int)(pass_len + 1u))) {
    ...
    for (uint j = 0; j < dict_length; ++j) {
      ...
      if (*g_found) return;
      if (prcrc32_compare(k_hash, attempt, (int)(pass_len + 2u))) {

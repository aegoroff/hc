/* Shared SHA-224/256 BF kernel. Heads must define:
 *   HASH_WORDS, SHA_IV0..SHA_IV7, SHA_HASH, SHA_COMPARE, SHA_KERNEL
 */
...
static void SHA_HASH(const uchar* message, uint len, uint* hash) {
  ...
  state[6] = SHA_IV6;
  state[7] = SHA_IV7;
  prsha256_process(state, message, len);
  for (int i = 0; i < HASH_WORDS; ++i) hash[i] = state[i];
}

static int SHA_COMPARE(__global const uchar* k_hash, uchar* password, const int length) {
  uint hash[HASH_WORDS];
  SHA_HASH(password, (uint)length, hash);
  int result = 1;
  for (int i = 0; i < HASH_WORDS && result; ++i) {
    result &= hash[i] == ((uint)k_hash[3 + i * 4] | (uint)k_hash[2 + i * 4] << 8 | (uint)k_hash[1 + i * 4] << 16 | (uint)k_hash[0 + i * 4] << 24);
  }
  return result;
}

__kernel void SHA_KERNEL(__global uchar* result,
  ...
  for (uint i = 0; i < dict_length; ++i) {
    ...
    if (pass_len + 1u == 4u && pass_len + 1u >= min_len) {
      if (*g_found) return;
      if (SHA_COMPARE(k_hash, attempt, (int)(pass_len + 1u))) {
    ...
    for (uint j = 0; j < dict_length; ++j) {
      ...
      if (*g_found) return;
      if (SHA_COMPARE(k_hash, attempt, (int)(pass_len + 2u))) {

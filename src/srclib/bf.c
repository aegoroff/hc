...
char* bf_brute_force(const uint32_t passmin,
    ...
    has_gpu_implementation = has_gpu_implementation && passmax > 3 && gpu_can_use_gpu();
    ...
    if(has_gpu_implementation) {
        ...
        for(i = 0; i < num_of_gpu_threads; ++i) {
            ...
            // 128 times more then max device blocks number
            gpu_thd_ctx[i]->max_gpu_blocks_number_ = gpu_props->max_blocks_number * 128;

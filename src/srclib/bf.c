...
static char* prbf_str_replace(char* orig, char* rep, char* with, apr_pool_t* pool);
...
char* prbf_str_replace(char* orig, char* rep, char* with, apr_pool_t* pool) {
    ...
    tmp = result = (char*)apr_pcalloc(pool, strlen(orig) + (len_with - len_rep) * count + 1);

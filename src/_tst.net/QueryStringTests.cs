...
namespace _tst.net
{
        ...
        where T : Architecture, new()
    {
        ...
        private IList<string> RunQuery(string template, params object[] parameters)
        {
            return this.Runner.Run(QueryOpt, string.Format(template, parameters), NoProbeOpt);
        }

        private IList<string> RunQueryWithOpt(string template, string additionalOptions, params object[] parameters)
        {
        ...
        {
            return this.RunQuery(HashStringCrackQueryTpl, h.HashString, h.Algorithm);
        }

        protected override IList<string> RunStringCrackTooShort(Hash h)
        {
            return this.RunQuery("for string s from hash '{0}' let s.max = {1} do crack {2};", h.HashString, h.InitialString.Length - 1, h.Algorithm);
        }

        protected override IList<string> RunStringCrackTooMinLength(Hash h)
        {
            return this.RunQuery("for string s from hash '{0}' let s.min = {1}, s.max = {2}, s.dict = '123' do crack {3};", h.HashString, h.InitialString.Length + 1, h.InitialString.Length + 2, h.Algorithm);
        }
        
        protected override IList<string> RunStringCrackLowCaseHash(Hash h)
        {
            return this.RunQuery("for string s from hash '{0}' let s.dict = '{2}' do crack {1};", h.HashString.ToLowerInvariant(), h.Algorithm, h.InitialString);
        }
        ...
        {
            return this.RunQueryWithOpt(HashStringQueryTpl, LowerOpt, h.InitialString, h.Algorithm);
        }
        ...
        {
            return this.RunQuery("for string s from hash '{0}' let s.dict = '{1}', s.max = 2, s.min = 2 do crack {2};", h.StartPartStringHash, dict, h.Algorithm);
        }
        ...
        public static IEnumerable<object[]> HashesAndNonDefaultDictSmall => CreateProperty(new object[] { "123", "0-9" });
    }
}
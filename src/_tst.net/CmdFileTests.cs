...
namespace _tst.net
{
        ...
        where T : Architecture, new()
    {
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, limitOptions);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, HashOpt, h.HashString);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, HashOpt, h.TrailPartStringHash);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, TimeOpt);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, LimitOpt, "2");
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, OffsetOpt, "1");
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, LimitOpt, "1", OffsetOpt, "1");
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, option, limit);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, LimitOpt, limit, OffsetOpt, offset);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, notEmptyFile, OffsetOpt, "4");
        ...
        public void CalcBigFileWithOffset(Hash h)
        {
            ...
            try
            {
                var results = this.Runner.Run(h.Algorithm, FileOpt, file, OffsetOpt, "1024");
        ...
        public void CalcBigFileWithLimitAndOffset(Hash h)
        {
            ...
            try
            {
                var results = this.Runner.Run(h.Algorithm, FileOpt, file, OffsetOpt, "1024", LimitOpt, "1048500");
                Assert.Contains(" Mb (2", results[0]);
                Assert.Equal(1, results.Count);
            }
            finally
        ...
        public void CalcUnexistFile(Hash h)
        {
            ...
            var results = this.Runner.Run(h.Algorithm, FileOpt, unexist);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, FileOpt, emptyFile);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir);
            Assert.Equal(string.Format(FileResultTpl, emptyFile, h.EmptyStringHash, 0), results[0]);
        ...
        public void CalcDirOutputToFile(Hash h)
        {
            ...
            try
            {
                var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, "-o", save);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, "--sfv");
        ...
        public void CalcDirRecursivelyManySubs(Hash h)
        {
            ...
            try
            {
                var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, RecurseOpt);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, IncludeOpt, EmptyFileName);
            Assert.Equal(string.Format(FileResultTpl, emptyFile, h.EmptyStringHash, 0), results[0]);
            Assert.Equal(1, results.Count);
        }
        ...
        {
            var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, ExcludeOpt, EmptyFileName);
            Assert.Equal(string.Format(FileResultTpl, notEmptyFile, h.HashString, h.InitialString.Length), results[0]);
            Assert.Equal(1, results.Count);
        }
        ...
        {
            var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, SearchOpt, h.HashString);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, SearchOpt, h.HashString, TimeOpt);
        ...
        {
            var results = this.Runner.Run(h.Algorithm, DirOpt, FileFixture.BaseTestDir, SearchOpt, h.HashString, RecurseOpt);
            Assert.Equal(2, results.Count);
        }
    }
}
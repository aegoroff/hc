...
// relation opcodes are NULL), so this is dormant. pcre2 is linked and exposed
// via the `re` translate-c module; the binding below ports processor.c once a
// processor slot dispatches to it.
...
pub fn matchRe(pattern: []const u8, subject: []const u8) bool {
    const compile_ctx = re.pcre2_compile_context_create(null) orelse return false;
    defer _ = re.pcre2_compile_context_free(compile_ctx);

    var errnumber: c_int = 0;
    var erroffset: usize = 0;
    const compiled = re.pcre2_compile(
        pattern.ptr,
        pattern.len,
        0,
        &errnumber,
    ...
    defer _ = re.pcre2_code_free(compiled);

    const match_data = re.pcre2_match_data_create_from_pattern(compiled, null) orelse return false;
    defer _ = re.pcre2_match_data_free(match_data);
    ...
    const rc = re.pcre2_match(compiled, subject.ptr, subject.len, 0, flags, match_data, null);
    return rc >= 0;
}

...
void gpu_run_wide(gpu_tread_ctx_t* ctx, const size_t dict_len, wchar_t* variants, const size_t variants_size, void(*pfn_kernel)(gpu_tread_ctx_t* c, wchar_t* r, wchar_t* v, const size_t dl)) {
    wchar_t* dev_result = nullptr;
    wchar_t* dev_variants = nullptr;

    size_t result_size_in_bytes = GPU_ATTEMPT_SIZE * sizeof(wchar_t); // include trailing zero

    CUDA_SAFE_CALL(cudaMalloc(reinterpret_cast<void**>(&dev_variants), variants_size * sizeof(wchar_t)));
    CUDA_SAFE_CALL(cudaMemcpyAsync(dev_variants, variants, variants_size * sizeof(wchar_t), cudaMemcpyHostToDevice));

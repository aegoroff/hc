...
__device__ static BOOL prsha512_compare(unsigned char* password, const int length);
...
__host__ void sha512_on_gpu_prepare(int device_ix, const unsigned char* dict, size_t dict_len, const unsigned char* hash, unsigned char** variants, size_t variants_len) {
...
__host__ void sha512_on_gpu_cleanup(gpu_tread_ctx_t* ctx) {
...
__global__ void prsha512_kernel(unsigned char* result, unsigned char* variants, const uint32_t dict_length) {
    ...
    if (prsha512_compare(attempt, len)) {
    ...
    for (uint32_t i = 0; i < dict_length; ++i)
    {
        ...
        if (prsha512_compare(attempt, attempt_len)) {
...
__device__ BOOL prsha512_compare(unsigned char* password, const int length) {
    //uint64_t hash[STATE_LEN];
    uint64_t* hash = (uint64_t*)malloc(STATE_LEN * sizeof(uint64_t));
